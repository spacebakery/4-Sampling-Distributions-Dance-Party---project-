...
def choose_statistic(x, sample_stat_text):
  ...
  # calculate variance if the text is "Variance"
    ...
    return np.var(x, ddof=1)    # default np.var(x, ddof=0) without applying Bessel's Correction denominator=(n-1)
  # if you want to add an extra stat
    ...
    return x.mode()[0]
  # raise error if sample_stat_text is not "Mean", "Median", "Variance", "Mode", "Minimum", "Maximum"
  else:
...
def population_distribution(population_data):
  ...
  # remove None label
  plt.xlabel('')
  plt.axvline(np.mean(population_data), c='r', label='Pop Mean', ls='--')
  plt.legend()
  plt.show()
  plt.clf()
